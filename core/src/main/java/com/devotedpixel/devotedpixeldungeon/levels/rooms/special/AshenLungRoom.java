...
import com.shatteredpixel.shatteredpixeldungeon.actors.mobs.AshenLung;
...
public class AshenLungRoom extends SpecialRoom {
	@Override
	public void paint(Level level) {
		...
		AshenLung spawner = new AshenLung();
	...
	public static class CustomFloor extends CustomTilemap {
		...
		public Tilemap create() {
			...
			for (int i = 0; i < data.length; i++){
				...
				if (Dungeon.level.findMob(cell) instanceof AshenLung){
					data[i-1] = 5 + 4*8;
					data[i] = 6 + 4*8;
					data[i+1] = 7 + 4*8;
					i++;
