...
public class VoidBombTracker extends Buff {
	...
	public boolean act() {
		...
		if (turnsLeft <= 0){
			...
			//bomb doesnt harm enemies ( duh )
				for (Char ch : Actor.chars()) {
					if (ch.alignment == Char.Alignment.ENEMY) {
						affect(ch, NovaBombImmune.class, 0f);
					}
				}
			...
			for (int i = 0; i < Dungeon.level.length(); i++){
				if (fieldOfView[i]){
					CellEmitter.get(i).burst(PitfallParticle.FACTORY8, 10);
				}
			}
		}
