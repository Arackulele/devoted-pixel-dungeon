...
public class RockGolem extends Mob {
	...
	public void damage(int dmg, Object src) {
		...
		if (reduce) {
			if (Dungeon.level.distance(foe.pos, pos) > 3) newdmg = 0;
			else if (Dungeon.level.distance(foe.pos, pos) == 2) newdmg = (int)(dmg*0.67);
			else if (Dungeon.level.distance(foe.pos, pos) == 3) newdmg = (int)(dmg*0.34);
		}
