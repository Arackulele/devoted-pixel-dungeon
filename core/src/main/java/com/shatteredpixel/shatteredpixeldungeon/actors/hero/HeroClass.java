...
import com.shatteredpixel.shatteredpixeldungeon.actors.hero.abilities.zealot.CallingBeyond;
...
import com.shatteredpixel.shatteredpixeldungeon.items.weapon.melee.Warpick;
...
public enum HeroClass {
	...
	ZEALOT( HeroSubClass.CONJURER, HeroSubClass.DEVOTEE );
	...
	private static void initZealot( Hero hero ) {
		...
		(hero.belongings.weapon = new Warpick()).identify();
		hero.belongings.weapon.activate(hero);
		...
		WoodenBoomerang rang = new WoodenBoomerang();
		rang.quantity(1).identify().collect(); //set quantity is 3, but Zealot starts with 1

		Dungeon.quickslot.setSlot(0, vial);
		Dungeon.quickslot.setSlot(1, rang);
	...
	public ArmorAbility[] armorAbilities(){
		switch (this) {
			...
			case ZEALOT:
				return new ArmorAbility[]{new CallingBeyond(), new EternalBalance(), new WealAndWoe()};
		}
	}
