...
public class Hero extends Char {
    ...
    public int attackSkill(Char target) {
        ...
        if (Dungeon.isChallenged(Challenges.REGION_DIFFS) && Dungeon.currentRegion() == Dungeon.Region.FORGE) {
            float counter = 0;
            if (buff(ForgeLevelCounter.class) != null)
                counter = buff(ForgeLevelCounter.class).count();
    ...
    public void spend(float time) {
        if (Dungeon.isChallenged(Challenges.REGION_DIFFS) && Dungeon.currentRegion() == Dungeon.Region.FORGE)
            Buff.count(this, ForgeLevelCounter.class, time);
        else if (Dungeon.isChallenged(Challenges.REGION_DIFFS) && Dungeon.depth > 15 && Dungeon.depth < 21)
            GetRegionArenaTiles(true);

        if (Dungeon.isChallenged(Challenges.REGION_DIFFS) && Dungeon.currentRegion() == Dungeon.Region.HALLS)
    ...
    public void damage(int dmg, Object src) {
        ...
        if (pointsInTalent(Talent.HIDDEN_POWER) != 0) {
            if (dmg >= HT / 5)
                Buff.affect(this, Invisibility.class, 1f + pointsInTalent(Talent.HIDDEN_POWER));
        }
        ...
        if (hasTalent(Talent.ESOTERIC_MIND)) {
            ...
            if (ref != null) {
                ...
                if (pointsInTalent(Talent.ESOTERIC_MIND) == 1 && max == 3)
                    dmg -= Random.NormalIntRange(0, max);
                else if ((pointsInTalent(Talent.ESOTERIC_MIND) == 2))
                    dmg -= Random.NormalIntRange(0, max);
            }
        }
    ...
    public boolean isHungry() {
        return Buff.affect(this, Hunger.class).isHungry();
    }
