...
public class Hero extends Char {
	...
	public int STR() {
        ...
        if (Buff.affect(this, Thirst.class).isParched()) strBonus -= 3;
        else if ((Buff.affect(this, Thirst.class).isThirsty())) strBonus -= 1;
    ...
    public boolean GetRegionArenaTiles(boolean visuals)
    {
        ArrayList<Integer> AffectedCells = GetRoomTiles();
        ...
        if (visuals) {
            for (int cell : AffectedCells) {
                if (Dungeon.level.heroFOV[cell]) {
                    if (Dungeon.currentRegion() == Dungeon.Region.CITADEL)
                        CellEmitter.get(cell - Dungeon.level.width()).start(CitadelLevel.BloodFlameParticle.FACTORY, 0.07f, 1);
                    else
                        CellEmitter.get(cell - Dungeon.level.width()).start(CityLevel.GreenFlameParticle.FACTORY, 0.07f, 1);
    ...
    public int attackSkill( Char target ) {
        ...
        if (Dungeon.isChallenged(Challenges.REGION_DIFFS) && Dungeon.currentRegion() == Dungeon.Region.FORGE)
        {
            float counter = 0;
            if (buff(ForgeLevelCounter.class) != null)counter = buff(ForgeLevelCounter.class).count();
            accuracy *= Math.max(1f - (counter * 0.004f), 0.5f);
        }
    ...
    public static class ForgeLevelCounter extends CounterBuff {};
    ...
    public int defenseSkill( Char enemy ) {
        ...
        if (Dungeon.isChallenged(Challenges.REGION_DIFFS) &&
                Dungeon.currentRegion() == Dungeon.Region.CAVES)
        {
            int Load = 0;
            if (belongings.weapon() != null) Load++;
            //Sorry duelist but its for consistency
            if (belongings.secondWep() != null) Load++;
            if (belongings.armor() != null) Load++;
            if (belongings.ring() != null) Load++;
            if (belongings.artifact() != null) Load++;
            if (belongings.misc() != null) Load++;
            switch (Load)
            {
                default: case 0: break;
                case 1: evasion *= 0.97f; break;
                case 2: evasion *= 0.86f; break;
                case 3: evasion *= 0.73f; break;
                case 4: evasion *= 0.62f; break;
                case 5: evasion *= 0.5f; break;
                case 6: evasion *= 0.34f; break;
            }
    ...
    public float speed() {
        ...
        if (Dungeon.isChallenged(Challenges.REGION_DIFFS) &&
                Dungeon.currentRegion() == Dungeon.Region.SEWERS &&
                Dungeon.level.water[pos]) speed *= 0.75f;
    ...
    public void spend( float time ) {
        if (Dungeon.isChallenged(Challenges.REGION_DIFFS) && Dungeon.currentRegion() == Dungeon.Region.FORGE) Buff.count(this, ForgeLevelCounter.class, time);
        else if (Dungeon.isChallenged(Challenges.REGION_DIFFS) && Dungeon.depth > 15 && Dungeon.depth < 21 )GetRegionArenaTiles(true);
    ...
    public boolean act() {
        ...
        if (curAction == null) {
            ...
            if (curAction instanceof HeroAction.Move) {
                ...
                Buff.detach(this, ForgeLevelCounter.class);
        ...
        if (Dungeon.isChallenged(Challenges.REGION_DIFFS) &&
            ...
            if (Dungeon.level.water[pos]) {
                WaterPoisonCounter count = Buff.count(this, WaterPoisonCounter.class, 1);
                if (count.count() > 11) Buff.affect(this, Poison.class).set(3f);
    ...
    public static class WaterPoisonCounter extends CounterBuff {};
    public static class GardensRootCounter extends CounterBuff {};
            ...
            if (heap != null) {
                ...
                if (item.doPickUp( this )) {
                    ...
                    if (item instanceof Dewdrop
                        ...
                        Ruby existing = belongings.getItem(Ruby.class);
                        if (existing != null){
                            if (existing.quantity() >= 40) {
                                GLog.p(Messages.get(Ruby.class, "you_now_have", existing.quantity()));
                            } else {
                                GLog.i(Messages.get(Ruby.class, "you_now_have", existing.quantity()));
                            }
                        }
                    }
                    else {
    ...
    public void damage( int dmg, Object src ) {
        ...
        if (hasTalent(Talent.ESOTERIC_MIND)) {

            Trinket ref = Dungeon.hero.belongings.getItem(Trinket.class);
            if (ref != null) {
                int max = ref.level();

                if (pointsInTalent(Talent.ESOTERIC_MIND) == 1 && max == 3) dmg -= Random.NormalIntRange(0, max);
                else if ((pointsInTalent(Talent.ESOTERIC_MIND) == 2)) dmg -= Random.NormalIntRange(0, max);
            }
        }
        ...
        if (Dungeon.level.adjacent( pos, target )) {
            ...
            if (newPath) {
				...
				for (int i = 0; i < len; i++) {
					...
					if (Dungeon.level instanceof LavaLakeLevel && Dungeon.level.map[i] == Terrain.WATER) passable[i] = false;
				}
    ...
    public void Heal(int amnt) {
        if (Dungeon.isChallenged(Challenges.REGION_DIFFS) && Dungeon.currentRegion() == Dungeon.Region.CITADEL && GetRegionArenaTiles(false)) {
            ArrayList<Mob> affected = new ArrayList<>();
            for (int cell : GetRoomTiles()) {
                Char a = Actor.findChar(cell);
                if (a != null && a instanceof Mob && a.alignment != alignment && a.isAlive())
                    affected.add((Mob) a);
            }
            //always heal a little more, to make the effect more noticable
            int heal = Math.max(amnt, 1) / (affected.size() + 1);
            for (Mob m : affected) {
                m.Heal(heal);
                m.sprite.emitter().burst(Speck.factory(Speck.HEALING), Math.max(1, heal / 10));
            }
        } else super.Heal(amnt);
    }
