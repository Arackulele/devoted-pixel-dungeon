...
import com.shatteredpixel.shatteredpixeldungeon.sprites.WretchSprite;
import com.shatteredpixel.shatteredpixeldungeon.utils.GLog;
import com.watabou.noosa.audio.Sample;
import com.watabou.utils.*;

public class Wretch extends NPC {

	{
		...
		spriteClass = WretchSprite.class;

		if (Dungeon.hero != null) HP = HT =22 + Dungeon.hero.lvl * 4;
		else HP = HT = 5;
		...
		public boolean act( boolean enemyInFOV, boolean justAlerted ) {
				...
				sprite.jump(pos, leapPos, new Callback() {
					...
					public void call() {
						...
						if (leapVictim != null && alignment != leapVictim.alignment){
							if (Char.hit(Wretch.this, leapVictim, Char.INFINITE_ACCURACY, false)) {
								Buff.prolong(leapVictim, Weakness.class, 10f);
						...
						if (endPos != leapPos){
							Actor.add(new Pushing(Wretch.this, leapPos, endPos));
						}

						pos = endPos;
						leapPos = -1;
						sprite.idle();
						Dungeon.level.occupyCell(Wretch.this);
						next();
					}
				});
				return false;
			...
			if (enemyInFOV && !isCharmedBy( enemy ) && canAttack( enemy )) {
				...
				if (enemyInFOV) {
					...
					state = WANDERING;
					target = Dungeon.level.randomDestination( Wretch.this );
					return true;
				}
				...
				if (leapCooldown <= 0 && enemyInFOV && !rooted
					...
					if (b.collisionPos == targetPos){
						...
						if (Dungeon.level.heroFOV[pos] || Dungeon.level.heroFOV[leapPos]){
							GLog.w(Messages.get(Wretch.this, "leap"));
							sprite.parent.addToBack(new TargetedCell(leapPos, 0xFF0000));
							((WretchSprite)sprite).leapPrep( leapPos );
