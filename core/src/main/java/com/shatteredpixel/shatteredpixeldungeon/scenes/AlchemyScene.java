...
public class AlchemyScene extends PixelScene {
	...
	public void create() {
		...
		if (splitAlchGuide &&
				...
				Camera.main.height >= MIN_HEIGHT_FULL){
		...
		RenderedTextBlock desc = renderTextBlock(6);
		desc.maxWidth(w);
		...
		energyLeft = renderTextBlock(energyText, 9);
		energyLeft.setPos(
		...
		StyledButton btnGuide = new StyledButton( Chrome.Type.TOAST_TR, Messages.get(AlchemyScene.class, "guide")){
			...
			protected void onClick() {
				super.onClick();
				if (Camera.main.width >= 300 && Camera.main.height >= MIN_HEIGHT_FULL){
