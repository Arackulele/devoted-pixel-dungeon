...
public class AboutScene extends PixelScene {
	...
	private static class CreditsBlock extends Component {
		...
		private CreditsBlock(boolean large, int highlight, String title, Image avatar, String body, String linkText, String linkUrl){
			...
			if (title != null) {
				this.title = renderTextBlock(title, large ? 8 : 6);
			...
			this.body = renderTextBlock(body, 6);
			...
			if (linkText != null && linkUrl != null){
				...
				this.link = renderTextBlock(linkText, 6);
		...
		protected void layout() {
			...
			if (link != null){
				...
				linkUnderline.size(link.width(), align(0.49f));
