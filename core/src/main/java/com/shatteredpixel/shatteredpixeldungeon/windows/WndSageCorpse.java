...
public class WndSageCorpse extends Window {
	...
	SageCorpse sagecorpse;
	Item questItem;

	public WndSageCorpse(final SageCorpse sageCorpse, final Item item ) {
		
		super();

		this.sagecorpse = sageCorpse;
		this.questItem = item;
		...
		String msg = "";
		if (item instanceof WandOfMalaise){
			msg = Messages.get(this, "wand");
		} else if (item instanceof EndothermicRing){
			msg = Messages.get(this, "ring");
		} else if (item instanceof CloakOfThorns){
			msg = Messages.get(this, "artifact");
		}
        ...
        if (SageCorpse.Quest.item1 == null || SageCorpse.Quest.item2 == null ) SageCorpse.Quest.GenerateItems();
	...
	private void selectReward( Item reward ) {
		...
		hide();
        if ( questItem.isEquipped(Dungeon.hero) )((EquipableItem)questItem).doUnequip(Dungeon.hero, true);
		...
		if (reward.doPickUp( Dungeon.hero )) {
			...
			Dungeon.level.drop( reward, sagecorpse.pos ).sprite.drop();
		}

		sagecorpse.yell( Messages.get(this, "farewell", Messages.titleCase(Dungeon.hero.name())) );
		sagecorpse.destroy();

		sagecorpse.sprite.die();

		SageCorpse.Quest.complete();
	}
	...
	private class RewardWindow extends WndInfoItem {
		...
		public RewardWindow( Item item ) {
			...
			RedButton btnConfirm = new RedButton(Messages.get(WndSadGhost.class, "confirm")){
				@Override
				protected void onClick() {
					WndSageCorpse.RewardWindow.this.hide();
