...
public class ScreamParticle extends PixelParticle.Shrinking {
	
	public static final Factory MISSILE = new Factory() {
		@Override
		public void emit( Emitter emitter, int index, float x, float y ) {
			((ScreamParticle)emitter.recycle( ScreamParticle.class )).reset( x, y );
		}
	};
	...
	public static final Factory CURSE = new Factory() {
		@Override
		public void emit( Emitter emitter, int index, float x, float y ) {
			((ScreamParticle)emitter.recycle( ScreamParticle.class )).resetCurse( x, y );
		}
	};
	...
	public static final Factory UP = new Factory() {
		@Override
		public void emit( Emitter emitter, int index, float x, float y ) {
			((ScreamParticle)emitter.recycle( ScreamParticle.class )).resetUp( x, y );
		}
	};
	...
	public void update() {
		...
		color( ColorMath.interpolate( 0x444444, 0xFFFFFF, p ) );
