...
public class Bomb extends Item {
	...
	public void explode(int cell){
		...
		if (explodesDestructively()) {
			...
			BArray.or( Dungeon.level.bombable, explodable, explodable);
			...
			for (int i : affectedCells){
				...
				if (Dungeon.level.flamable[i] || Dungeon.level.bombable[i]) {
			...
			for (Char ch : affectedChars){
				...
				if (dmg > 0) {
					...
					if (Dungeon.hero.hasTalent(Talent.DEMOLITIONIST))
					{
						...
						switch (Dungeon.hero.pointsInTalent(Talent.DEMOLITIONIST)) {
							case 1: max = 30;
							case 2: max = 40;
							case 3: max = 50;
						}
						...
						if (Dungeon.hero.buff(Barrier.class) != null) amnt += Dungeon.hero.buff(Barrier.class).shielding();

						max -= amnt;


						if (max > 0) {
							Buff.affect(Dungeon.hero, Barrier.class).setShield(Math.min(max, dmg));
						}
