...
public class Tomahawk extends MissileWeapon {
    ...
    public int proc( Char attacker, Char defender, int damage ) {
        ...
        Buff.affect( defender, Bleeding.class ).set( augment.damageFactor((int) Random.NormalFloat(minBleed(), maxBleed())) );
    ...
    public String statsInfo(){
        if (isIdentified()){
            return Messages.get(this, "stats_desc",
                    Math.round(augment.damageFactor((int) minBleed())),
                    Math.round(augment.damageFactor((int) maxBleed())));
        } else {
            return Messages.get(this, "typical_stats_desc",
                    Math.round(augment.damageFactor((int) minBleed(0))),
                    Math.round(augment.damageFactor((int) maxBleed(0))));
        }
    }
