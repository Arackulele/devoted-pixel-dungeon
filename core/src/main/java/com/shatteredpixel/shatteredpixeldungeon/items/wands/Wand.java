...
public abstract class Wand extends Item {
	...
	protected static void wandProc(Char target, int wandLevel, int chargesUsed){
		...
		if (Dungeon.hero.hasTalent(Talent.CONFIRMED_BELIEF) && Dungeon.hero.buff(Invulnerability.class) != null) {
			float dur;
			if (Dungeon.hero.pointsInTalent(Talent.CONFIRMED_BELIEF) == 1) dur = 0.67f;
			else dur = 1;

			Buff.prolong(Dungeon.hero, Invulnerability.class, Dungeon.hero.buff(Invulnerability.class).cooldown() +dur);
		}
	...
	public class Charger extends Buff {
		...
		private void recharge(){
			if (wand().getClass() == WandOfMalaise.class) return;
