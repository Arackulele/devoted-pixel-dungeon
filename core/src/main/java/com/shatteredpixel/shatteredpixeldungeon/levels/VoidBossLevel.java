...
public class VoidBossLevel extends Level {
    ...
    //ToDo: Saving for this, and the combos for the court, and the sacrifice, and the frogeat
	...
	public int randomRespawnCell( Char ch ) {
        if (!(ch instanceof Hero)) return super.randomRespawnCell(ch);
	...
	public void restoreFromBundle(Bundle bundle) {
		...
		for (Mob m : mobs){
			if (m instanceof VelTaleth){
				((VelTaleth) m).updateVisibility(this);
			}
		}
	}
					...
					GameScene.show( new WndOptions( new ItemSprite(ItemSpriteSheet.AMULET),
						...
						protected void onSelect(int index) {
							if (index == 0){
								...
								VoidBossLevel.super.activateTransition(hero, transition);
							}
						}
					} );
				}
	...
	public Group addVisuals () {
		super.addVisuals();
		VoidLevel.addHallsVisuals( this, visuals );
		return visuals;
	}
